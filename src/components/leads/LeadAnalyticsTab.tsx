...
const PANEL = 'rounded-2xl border border-[color:var(--hub-card-border)] bg-[rgba(0,0,0,0.22)] overflow-hidden'
const SECTION_LABEL = 'text-[11px] font-bold uppercase tracking-widest text-[color:var(--hub-desc)]'
const MUTED = 'text-[color:var(--hub-desc)]'
const FIELD = 'w-full rounded-xl border border-[color:var(--hub-card-border)] bg-[rgba(0,0,0,0.35)] px-4 py-3 text-sm text-[color:var(--app-text)] placeholder:text-[color:var(--shell-search-ph)] outline-none focus:border-[color:var(--hub-card-border-hover)] transition-all'
...
const FLABEL = 'block text-xs font-semibold uppercase tracking-wide text-[color:var(--hub-stat-label)] mb-2'

const SELECT_TRIGGER = 'h-11 rounded-xl border border-[color:var(--hub-card-border)] bg-[rgba(0,0,0,0.35)] text-[color:var(--app-text)] shadow-none focus:ring-1 focus:ring-[color:var(--hub-card-border)] focus:border-[color:var(--hub-card-border-hover)] [&>span]:text-[color:var(--app-text)]'
const SELECT_CONTENT = 'border-[color:var(--hub-card-border)] bg-[rgba(9,36,28,0.98)] text-[color:var(--app-text)] backdrop-blur-sm'
...
function KpiCard({ label, value, sub, subTooltip, icon, trend }: {
  ...
  return (
    <div className="rounded-2xl border border-[color:var(--hub-card-border)] bg-[rgba(0,0,0,0.22)] p-5 flex flex-col items-center text-center gap-3">
      <div className="flex items-center gap-2">
        <div className="flex size-10 items-center justify-center rounded-xl bg-[var(--hub-tile-icon-bg)] text-[color:var(--hub-body)]">
          {icon}
        </div>
        {trend && (
          trend === 'up'
      ...
      <div>
        <p className="text-3xl font-bold text-[color:var(--app-text)] tabular-nums">{value}</p>
        <p className="text-sm text-[color:var(--hub-stat-label)] mt-1">{label}</p>
        {sub && (subTooltip ? (
          <Tooltip>
            <TooltipTrigger asChild>
              <p className="text-xs text-[color:var(--theme-accent-icon-dim)] mt-0.5 cursor-help underline decoration-dotted decoration-[color:var(--hub-tile-icon-border)] underline-offset-2">{sub}</p>
            </TooltipTrigger>
          ...
          <p className="text-xs text-[color:var(--theme-accent-icon-dim)] mt-0.5">{sub}</p>
        ))}
      </div>
    </div>
  )
  ...
  return (
    <div className={cn(PANEL, 'p-6 space-y-5')}>
      <div className="flex items-center gap-2.5">
        <Link className="size-4 text-[color:var(--hub-stat-label)]" />
        <p className="text-base font-semibold text-[color:var(--app-text)]">UTM-генератор</p>
      </div>
      ...
      <div className="rounded-xl border border-[color:var(--hub-card-border)] bg-[rgba(0,0,0,0.3)] px-4 py-3 flex items-center justify-between gap-3">
        <p className="text-xs text-[color:var(--hub-body)] truncate font-mono">{url}</p>
        <button type="button" onClick={copy}
          className="flex shrink-0 items-center gap-1.5 rounded-full border border-[color:var(--hub-card-border)] px-4 py-1.5 text-xs font-medium text-[color:var(--theme-accent-link-dim)] hover:border-[color:var(--hub-card-border-hover)] hover:text-[color:var(--app-text)] transition-colors">
...
function CampaignDialog({ open, campaign, onClose, onSave }: {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent showCloseButton={false} className="max-w-lg rounded-2xl border border-[color:var(--hub-card-border)] bg-[linear-gradient(180deg,rgba(9,36,28,0.99),rgba(6,20,16,0.98))] p-0 shadow-2xl">
        <div className="flex items-center justify-between border-b border-[color:var(--hub-tile-icon-border)] px-6 py-5">
        ...
        <div className="space-y-5 px-6 py-6">
            ...
            <div>
              ...
              <Select value={form.channel} onValueChange={v => set('channel', v as CampaignChannel)}>
                ...
                <SelectContent className={SELECT_CONTENT}>
                  {CHANNELS.map(ch => (
                    <SelectItem key={ch} value={ch} className="text-[color:var(--app-text)] focus:bg-[var(--hub-tile-icon-bg)] focus:text-[color:var(--app-text)]">{ch}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.startDate} onChange={e => set('startDate', e.target.value)} type="date" className={cn(FIELD, 'text-[color:var(--app-text)] [color-scheme:dark]')} />
            </div>
            <div>
              <label className={FLABEL}>Дата окончания</label>
              <input value={form.endDate} onChange={e => set('endDate', e.target.value)} type="date" className={cn(FIELD, 'text-[color:var(--app-text)] [color-scheme:dark]')} />
            </div>
          </div>
          <div className="rounded-xl border border-[color:var(--hub-tile-icon-border)] bg-[rgba(0,0,0,0.25)] p-4 space-y-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-[color:var(--hub-stat-label)]">UTM-метки (необязательно)</p>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="text-[11px] text-[color:var(--hub-stat-label)] mb-1.5 block">utm_source</label>
                <input value={form.utmSource} onChange={e => set('utmSource', e.target.value)} placeholder="yandex" className={cn(FIELD, 'py-2')} />
              </div>
              <div>
                <label className="text-[11px] text-[color:var(--hub-stat-label)] mb-1.5 block">utm_medium</label>
                <input value={form.utmMedium} onChange={e => set('utmMedium', e.target.value)} placeholder="cpc" className={cn(FIELD, 'py-2')} />
              </div>
              <div>
                <label className="text-[11px] text-[color:var(--hub-stat-label)] mb-1.5 block">utm_campaign</label>
                <input value={form.utmCampaign} onChange={e => set('utmCampaign', e.target.value)} placeholder="brand" className={cn(FIELD, 'py-2')} />
              </div>
            </div>
          </div>
        </div>
        <div className="flex items-center justify-end gap-3 border-t border-[color:var(--hub-tile-icon-border)] px-6 py-4">
          <button type="button" onClick={onClose}
            className="rounded-full border border-[color:var(--hub-card-border)] px-5 py-2.5 text-sm font-medium text-[color:var(--hub-body)] hover:border-[color:var(--hub-card-border-hover)] hover:text-[color:var(--app-text)] transition-colors">
            Отмена
          </button>
  ...
  return (
    <div className={cn(
      ...
      isTarget
        ...
        : 'border border-[color:var(--hub-card-border)] bg-[rgba(0,0,0,0.15)]',
    )}>
      <div className="flex items-center justify-between gap-2 mb-1.5">
        <div className="flex items-center gap-1.5 min-w-0">
          <span className={cn(
            'text-xs truncate',
            isTarget ? 'font-medium text-emerald-300' : isRejection ? 'text-red-300/80' : 'text-[color:var(--theme-accent-link-dim)]',
          )}>{name}</span>
          {targetLabel && (
        ...
        <span className={cn(
          ...
          isTarget ? 'text-emerald-300' : isRejection ? 'text-red-300/80' : 'text-[color:var(--app-text)]',
        )}>
          {count}{' '}
          <span className="font-normal text-[color:var(--theme-accent-icon-dim)]">{pct}%</span>
        </span>
      </div>
      <div className="h-1 w-full overflow-hidden rounded-full bg-[var(--nav-item-bg-active)]">
        <div
          className={cn(
            'h-full rounded-full transition-all',
            isTarget ? 'bg-emerald-500/60' : isRejection ? 'bg-red-400/40' : 'bg-[var(--gold)]/30',
          )}
...
export function LeadAnalyticsTab() {
  ...
  const sourceBreakdown = useMemo(() => {
    ...
    return [
      ...
      { label: 'Другое',    count: other,     pct: total > 0 ? Math.round(other     / total * 100) : 0, color: 'bg-[var(--gold)]/35' },
    ]
  }, [leadPool])
    ...
    <div className="space-y-10">
        ...
        <div className={PANEL}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[color:var(--hub-tile-icon-border)]">
                  {['Кампания', 'Канал', 'Показы', 'Клики', 'CTR', 'Лиды', 'CPL', 'Бюджет', 'Статус', ''].map((h, i) => (
                    <th key={i} className={cn('px-5 py-4 text-[11px] font-bold uppercase tracking-wider text-[color:var(--hub-stat-label)]', i >= 2 && i <= 7 ? 'text-right' : 'text-left')}>
                      {h}
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-[color:var(--hub-action-hover)]">
                {campaigns.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className="group hover:bg-[var(--hub-action-hover)] transition-colors">
                      <td className="px-5 py-4">
                        <p className="text-[15px] font-semibold text-[color:var(--app-text)] max-w-[200px] truncate">{c.name}</p>
                        {c.utmCampaign && <p className="text-xs text-[color:var(--theme-accent-icon-dim)] font-mono mt-0.5">utm={c.utmCampaign}</p>}
                      </td>
                      <td className="px-5 py-4 text-sm text-[color:var(--hub-body)]">{c.channel}</td>
                      <td className="px-5 py-4 text-right text-sm tabular-nums text-[color:var(--app-text-muted)]">{c.impressions.toLocaleString('ru-RU')}</td>
                      <td className="px-5 py-4 text-right text-sm tabular-nums text-[color:var(--app-text-muted)]">{c.clicks.toLocaleString('ru-RU')}</td>
                      <td className="px-5 py-4 text-right text-sm tabular-nums text-[color:var(--app-text-muted)]">{ctr}%</td>
                      <td className="px-5 py-4 text-right text-base tabular-nums font-bold text-emerald-400">{c.leads}</td>
                      <td className="px-5 py-4 text-right text-sm tabular-nums text-[color:var(--app-text-muted)]">{cpl > 0 ? formatDollars(cpl) : '—'}</td>
                      <td className="px-5 py-4 text-right min-w-[130px]">
                        <p className="text-sm tabular-nums text-[color:var(--app-text-muted)]">{formatDollarsCompact(c.spent)} / {formatDollarsCompact(c.budget)}</p>
                        <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-[var(--hub-tile-icon-bg)]">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          {c.status !== 'ended' && (
                            <button type="button" onClick={() => toggleStatus(c.id)}
                              className="rounded-lg p-2 text-[color:var(--hub-desc)] hover:text-amber-400 hover:bg-amber-900/20 transition-colors"
                              title={c.status === 'active' ? 'Пауза' : 'Возобновить'}>
                              {c.status === 'active' ? <Pause className="size-4" /> : <Play className="size-4" />}
                            </button>
                          )}
                          <button type="button" onClick={() => openEdit(c)}
                            className="rounded-lg p-2 text-[color:var(--hub-desc)] hover:text-[color:var(--app-text)] hover:bg-[var(--nav-item-bg-active)] transition-colors">
                            <Pencil className="size-4" />
                          </button>
                          <button type="button" onClick={() => handleDelete(c.id)}
                            className="rounded-lg p-2 text-[color:var(--hub-desc)] hover:text-red-400 hover:bg-red-900/15 transition-colors">
                ...
                {campaigns.length === 0 && (
                  <tr><td colSpan={10} className="px-5 py-16 text-center text-[color:var(--theme-accent-icon-dim)]">Нет кампаний — нажмите «Добавить кампанию»</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className={cn(PANEL, 'p-6 space-y-4')}>
            <div className="flex items-center gap-2">
              <BarChart2 className="size-4 text-[color:var(--hub-desc)]" />
              <p className="text-sm font-semibold text-[color:var(--app-text)]">Источники лидов</p>
            </div>
            <div className="space-y-3">
              {sourceBreakdown.map(({ label, count, pct, color }) => (
                <div key={label}>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-sm text-[color:var(--theme-accent-link-dim)]">{label}</span>
                    <span className="text-sm tabular-nums font-semibold text-[color:var(--app-text)]">
                      {count}{' '}
                      <span className="text-xs font-normal text-[color:var(--workspace-text-muted)]">({pct}%)</span>
                    </span>
                  </div>
                  <div className="h-2 w-full overflow-hidden rounded-full bg-[var(--nav-item-bg-active)]">
          ...
          <div className={cn(PANEL, 'p-6 space-y-4')}>
            <div className="flex items-center gap-2">
              <TrendingUp className="size-4 text-[color:var(--hub-desc)]" />
              <p className="text-sm font-semibold text-[color:var(--app-text)]">Поступление рекламных лидов</p>
            </div>
              ...
              {weeklyTrend.map(({ label, count }, i) => {
                ...
                return (
                  <div key={i} className="flex flex-1 flex-col items-center gap-1">
                    <span className="text-[11px] tabular-nums text-[color:var(--hub-desc)]">{count || ''}</span>
                    <div className="flex w-full items-end rounded-t" style={{ height: 52 }}>
                      <div
                        className="w-full rounded-t-md bg-[var(--gold)]/25 hover:bg-[color-mix(in_srgb,var(--gold)_40%,transparent)] transition-colors"
                        style={{ height: `${heightPct}%`, minHeight: count > 0 ? 3 : 0 }}
                      />
                    </div>
                    <span className="text-[10px] text-[color:var(--theme-accent-icon-dim)] text-center leading-tight whitespace-nowrap">{label}</span>
                  </div>
                )
              })}
            </div>
      ...
      <section className="space-y-4">
        ...
        <div className={cn(PANEL, 'p-5 space-y-4')}>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-[color:var(--app-text)]">Воронка рекламных лидов</p>
              <p className="mt-0.5 text-xs text-[color:var(--theme-accent-icon-dim)]">{conversionFunnel.total} лидов в выборке</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(['7d', '30d', '90d', 'all'] as const).map(p => (
                <button
                  ...
                  className={cn(
                    ...
                    funnelPeriod === p
                      ? 'border-[color:var(--hub-card-border-hover)] bg-[var(--nav-item-bg-active)] text-[color:var(--app-text)]'
                      : 'border-[color:var(--hub-tile-icon-border)] text-[color:var(--hub-stat-label)] hover:border-[color:var(--hub-tile-icon-border)] hover:text-[color:var(--theme-accent-link-dim)]',
                  )}
                >
              ...
              <Select value={funnelCampaign} onValueChange={setFunnelCampaign}>
                ...
                <SelectContent className={SELECT_CONTENT}>
                  <SelectItem value="all" className="text-[color:var(--app-text)] focus:bg-[var(--hub-tile-icon-bg)] focus:text-[color:var(--app-text)]">Все кампании</SelectItem>
                  {campaigns.map(c => (
                    <SelectItem key={c.id} value={c.id} className="text-[color:var(--app-text)] focus:bg-[var(--hub-tile-icon-bg)] focus:text-[color:var(--app-text)]">
                      {c.name}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          {/* Старт */}
          <div className="flex items-center justify-between rounded-xl border border-[color:var(--hub-tile-icon-border)] bg-[var(--hub-action-hover)] px-4 py-3">
            <span className="text-sm font-semibold text-[color:var(--app-text)]">Новый лид</span>
            <div className="flex items-center gap-3">
              <span className="text-xl font-bold tabular-nums text-[color:var(--app-text)]">{conversionFunnel.newLeadsCount}</span>
              <span className="rounded-full border border-[color:var(--hub-card-border)] px-2.5 py-0.5 text-xs text-[color:var(--workspace-text-muted)]">100% · база</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 border-t border-[color:var(--hub-tile-icon-border)] pt-3">
              ...
              return (
                <div key={targetId} className="flex items-center justify-between rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-[11px] text-[color:var(--workspace-text-muted)]">{s.name}</p>
                  </div>
      ...
      {canViewNetworkAnalytics && (
        <section className="space-y-4">
          ...
          <div className={PANEL}>
            <div className="divide-y divide-[color:var(--hub-action-hover)]">
              <div className="px-5 py-4 flex items-center gap-4">
                <span className="inline-flex size-7 items-center justify-center rounded-full bg-[var(--hub-tile-icon-bg)] text-xs font-bold text-[color:var(--hub-badge-soon-fg)]">1</span>
                <span className="text-[15px] font-semibold text-[color:var(--app-text)]">Анна Первичкина</span>
                <span className="ml-auto text-base tabular-nums font-bold text-emerald-400">18%</span>
              </div>
              <div className="px-5 py-4 flex items-center gap-4">
                <span className="inline-flex size-7 items-center justify-center rounded-full bg-[var(--hub-tile-icon-bg)] text-xs font-bold text-[color:var(--hub-badge-soon-fg)]">2</span>
                <span className="text-[15px] font-semibold text-[color:var(--app-text)]">Борис Вторичкин</span>
                <span className="ml-auto text-base tabular-nums font-bold text-[color:var(--hub-badge-soon-fg)]">12%</span>
              </div>
            </div>
          </div>
        </section>
